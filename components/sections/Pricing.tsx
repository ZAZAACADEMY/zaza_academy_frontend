...
import { SparkleDoodle, ArrowDoodle } from "../ui/Doodles";
...
const PricingCard = ({
  ...
  `}
    ...
    >
      {isPremium && (
        <div className="absolute -top-5 left-0 right-0 flex justify-center z-20 pointer-events-none">
          ...
          >
            <SparkleDoodle className="w-4 h-4 text-white relative top-0 left-0" />
            {tLabel}
          </motion.div>
        </div>
      )}
    ...
    `}
      >
        <p className="font-bold text-[16px] mb-1">{installment}</p>
        <p
      ...
      <ul className="flex flex-col gap-4 mb-8 flex-grow w-full">
        {features.map((feature, i) => (
          <li key={i} className="flex items-start gap-3">
            ...
            ${isPremium ? "bg-white text-[#8B5CF6]" : "bg-[#A655F7] text-white"}
          `}
            >
...
export const Pricing = () => {
  ...
  const plans = (t.raw("plans") as PlanMessage[]).map((plan) => {
    const Icon = plan.icon ? (planIconMap[plan.icon] ?? Sparkles) : Sparkles;
    return {
      ...plan,
      icon: Icon,
      isPremium: Boolean(plan.mostPopular),
    };
  });
